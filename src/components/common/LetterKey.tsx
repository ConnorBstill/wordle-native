...
const LetterKey = ({ title }: { title: string }) => {
  const { guessedLetters, correctWord, guessedWords } = useAppSelector(state => state.letters);
  ...
  const setKeyBackground = (letter: string, letterIndex: number) => {
    ...
    guessedWords.forEach((word: string) => {
      if (correctWord.indexOf(title) !== -1 && correctWord.indexOf(title) === word.indexOf(title))  {
        color = GREEN;
